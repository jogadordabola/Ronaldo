...
| League Client (LCU) | Your profile, rank, match history, live game, mastery, item sets |
...
- **Other players' match history** is often restricted by Riot. Name, rank and mastery
  usually resolve; history may not. When it fails the app says so and logs the endpoints it
  tried to `%LOCALAPPDATA%\ronaldo\match-history-probe.txt`.
